...
import numpy.all
import numpy.round
from numpy import (argmin, array, asarray, atleast_1d, atleast_2d,
                   broadcast_arrays, empty, empty_like, float64, isreal,
                   isscalar, nan, ndarray, ndim, real, roots, where)
...
def fv(rate, nper, pmt, pv, when="end"):
    ...
    >>> a = array((0.05, 0.06, 0.07))/12
    >>> npf.fv(a, 10*12, -100, -100)
    array([15692.92889434, 16569.87435405, 17509.44688102])

    """
    when = _convert_when(when)
    rate, nper, pmt, pv, when = broadcast_arrays(rate, nper, pmt, pv, when)

    fv_array = empty_like(rate)
    zero = rate == 0
    nonzero = ~zero
    ...
    fv_array[nonzero] = -pv[nonzero] * temp - pmt[nonzero] * (
        1 + rate_nonzero * when[nonzero]
    ) / rate_nonzero * (temp - 1)
...
def pmt(rate, nper, pv, fv=0, when="end"):
    ...
    """
    when = _convert_when(when)
    (rate, nper, pv, fv, when) = map(array, [rate, nper, pv, fv, when])
    temp = (1 + rate) ** nper
    mask = rate == 0
    masked_rate = where(mask, 1, rate)
    fact = where(mask != 0, nper, (1 + masked_rate * when) * (temp - 1) / masked_rate)
...
def nper(rate, pmt, pv, fv=0, when="end"):
    ...
    >>> print(numpy.round(npf.nper(0.07/12, -150, 8000), 5))
    64.07335
    ...
    """
    when = _convert_when(when)
    rates = atleast_1d(rate).astype(float64)
    pmts = atleast_1d(pmt).astype(float64)
    pvs = atleast_1d(pv).astype(float64)
    fvs = atleast_1d(fv).astype(float64)
    whens = atleast_1d(when).astype(float64)
...
def _value_like(arr, value):
    ...
    return array(value, dtype=arr.dtype).item(0)
...
def ipmt(rate, per, nper, pv, fv=0, when="end"):
    ...
    >>> interestpd = numpy.sum(ipmt)
    >>> numpy.round(interestpd, 2)
    float64(-112.98)

    """
    when = _convert_when(when)
    rate, per, nper, pv, fv, when = broadcast_arrays(rate, per, nper, pv, fv, when)

    total_pmt = pmt(rate, nper, pv, fv, when)
    ipmt_array = array(_rbl(rate, per, total_pmt, pv, when) * rate)

    # Payments start at the first period, so payments before that
    # don't make any sense.
    ipmt_array[per < 1] = _value_like(ipmt_array, nan)
    ...
    ipmt_array[per_gt_1_and_begin] = ipmt_array[per_gt_1_and_begin] / (
        1 + rate[per_gt_1_and_begin]
    )

    if ndim(ipmt_array) == 0:
        # Follow the ufunc convention of returning scalars for scalar
        # and 0d array inputs.
...
def pv(rate, nper, pmt, fv=0, when="end"):
    ...
    """
    when = _convert_when(when)
    (rate, nper, pmt, fv, when) = map(asarray, [rate, nper, pmt, fv, when])
    temp = (1 + rate) ** nper
    fact = where(rate == 0, nper, (1 + rate * when) * (temp - 1) / rate)
...
def rate(
    ...
    *,
    ...
    (nper, pmt, pv, fv, when) = map(asarray, [nper, pmt, pv, fv, when])

    rn = guess
    iterator = 0
    close = False
    while (iterator < maxiter) and not numpy.all(close):
    ...
    if not numpy.all(close):
        if isscalar(rn):
            ...
            return default_type(nan)
        else:
            # Return nan's in array of the same shape as rn
            # where the solution is not close to tol.
            if raise_exceptions:
                raise IterationsExceededError(
                    f"Maximum iterations exceeded in "
...
def _irr_default_selection(eirr):
    ...
    same_sign = numpy.all(eirr > 0) if eirr[0] > 0 else numpy.all(eirr < 0)
...
def irr(values, *, raise_exceptions=False, selection_logic=_irr_default_selection):
    ...
    using irr in the Examples section below). Suppose one invests 100
    ...
    irr_results = empty(values.shape[0])
    for i, row in enumerate(values):
        # If all values are of the same sign, no solution exists
        # We don't perform any further calculations and exit early
        same_sign = numpy.all(row > 0) if row[0] > 0 else numpy.all(row < 0)
        if same_sign:
            if raise_exceptions:
                raise NoRealSolutionError(
...
def npv(rate, values):
    ...
    >>> numpy.round(npf.npv(rate, cashflows), 5)
    float64(3065.22267)
    ...
    >>> cashflows[0] = 0
    >>> numpy.round(npf.npv(rate, cashflows) + initial_cashflow, 5)
    float64(3065.22267)
    ...
    array([[-2700.  , -3500.  ],
           ...
           [-2884.3 , -3710.74]])
    """
    values_inner = atleast_2d(values).astype(float64)
    rate_inner = atleast_1d(rate).astype(float64)
...
def mirr(values, finance_rate, reinvest_rate, *, raise_exceptions=False):
    ...
    """
    values_inner = atleast_2d(values).astype(float64)
    finance_rate_inner = atleast_1d(finance_rate).astype(float64)
    reinvest_rate_inner = atleast_1d(reinvest_rate).astype(float64)
    ...
    for i, v in enumerate(values_inner):
        for j, (rr, fr) in enumerate(zip(reinvest_rate_inner, finance_rate_inner)):
            ...
            if not (pos.any() and neg.any()):
                if raise_exceptions:
                    raise NoRealSolutionError(
                        "No real solution exists for MIRR since"
                        " all cashflows are of the same sign."
                    )
                out[i, j] = nan
            else:
                numer = numpy.abs(npv(rr, v * pos))
                denom = numpy.abs(npv(fr, v * neg))
